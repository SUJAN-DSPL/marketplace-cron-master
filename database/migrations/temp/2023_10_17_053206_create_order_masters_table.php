...
class CreateOrderMastersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('mp_order_masters', function (Blueprint $table) {
            ...
            $table->unsignedSmallInteger('csr_id')->default(0);
            $table->unsignedInteger('cust_id');
            $table->unsignedSmallInteger('del_country_id');
            $table->unsignedSmallInteger('bill_country_id');
            $table->string('source');
            $table->unsignedSmallInteger('shipping_site_id');
            $table->unsignedSmallInteger('shipping_master_id');
            $table->unsignedSmallInteger('source_site_id');
            $table->mediumInteger('paneu_vat_cm_id')->comment('For PANEU vat country  Master ID');
            $table->unsignedSmallInteger('del_master_country_id')->default(0);
